...
class InMap_Shortcode extends Joe_v1_0_Shortcode {
	...
	function load_assets() {
		//InMap CSS
		Joe_v1_0_Assets::css_enqueue(Joe_v1_0_Helper::plugin_url('assets/css/shortcode.min.css'));	
		
		//Message Icon
		if($message_icon = Joe_v1_0_Config::get_setting('appearance', 'icons', 'message_icon')) {
			Joe_v1_0_Assets::css_inline('
				/* Icons */
		...
		//Tracking Icon
		if($tracking_icon = Joe_v1_0_Config::get_setting('appearance', 'icons', 'tracking_icon')) {
			Joe_v1_0_Assets::css_inline('
		...
		$primary_colour = Joe_v1_0_Config::get_setting('appearance', 'colours', 'tracking_colour');
		if($primary_colour) {
			Joe_v1_0_Assets::css_inline('
				/* Colours */
		...
		Joe_v1_0_Assets::js_inline('
			...
			if(typeof L !== "object" || L.version.indexOf("1") !== 1) {			
				...
				jQuery("head")
					.append(
						jQuery("<link />").attr({
							"href" : "' . Joe_v1_0_Helper::plugin_url('assets/css/leaflet.min.css') . '",
							"rel" : "stylesheet",
					...
					.append(
						jQuery("<script />").attr({
							"id" : "inmap_leaflet_js",					
							"src" : "' . Joe_v1_0_Helper::plugin_url('assets/js/leaflet.min.js') . '",
...
// 		]);
		...
		Joe_v1_0_Assets::js_enqueue([
			...
			'url' => Joe_v1_0_Helper::plugin_url('assets/js/shortcode.min.js'),
  			'deps' => [ 'jquery' ],
			'data' => [
				'basemap_url' => Joe_v1_0_Config::get_setting('appearance', 'map', 'basemap_url'),
				'basemap_attribution' => Joe_v1_0_Config::get_setting('appearance', 'map', 'basemap_attribution')
			]
		]);		
	}
	...
	public function handle_shortcode($shortcode_data, $content = null) {
		...
		$out = "\n" . '<!-- START ' . Joe_v1_0_Config::get_name() . ' Shortcode -->' . "\n";
		...
		if($shortcode_data['mapshare_identifier']) {					
			...
			//Error?
			if($error = Joe_v1_0_Log::in_error()) {
				Joe_v1_0_Log::render_item($error, 'console');			
			//Proceed
			} else {
				//Create *unqiue* Hash used to target Div
				$hash = Joe_v1_0_Helper::make_hash(
					array_merge(
						$Inreach_Mapshare->get_parameters(),
						//Salty count
						[
							'count' => Joe_v1_0_Log::get_data('shortcode_count')						
						]
					)
				);
				...
				if(is_string($geojson) && ! empty($geojson)) {		
					Joe_v1_0_Log::add(sprintf('Displaying %s MapShare Points.', $Inreach_Mapshare->get_point_count()), 'success', 'rendering_points');
					...
					$shortcode_count = (int)Joe_v1_0_Log::get_data('shortcode_count');
					$shortcode_count++;
		...
		$out .= '</div>';
		$out .= '<!-- END ' . Joe_v1_0_Config::get_name() . ' Shortcode -->' . "\n\n";
		...
		if(current_user_can('administrator')) {
			Joe_v1_0_Log::render();
		//Error?
		} elseif($error = Joe_v1_0_Log::in_error()) {
			Joe_v1_0_Log::render_item($error);
		}
