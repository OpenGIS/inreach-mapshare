...
class InMap_Shortcode extends Joe_v1_2_Shortcode {
	...
	function load_assets() {
		//InMap CSS
		Joe_v1_2_Assets::css_enqueue(Joe_v1_2_Helper::plugin_url('assets/css/shortcode.min.css'));	
		
		//Message Icon
		if($message_icon = Joe_v1_2_Config::get_setting('appearance', 'icons', 'message_icon')) {
			Joe_v1_2_Assets::css_inline('
				/* Icons */
		...
		//Tracking Icon
		if($tracking_icon = Joe_v1_2_Config::get_setting('appearance', 'icons', 'tracking_icon')) {
			Joe_v1_2_Assets::css_inline('
		...
		$primary_colour = Joe_v1_2_Config::get_setting('appearance', 'colours', 'tracking_colour');
		if($primary_colour) {
			Joe_v1_2_Assets::css_inline('
				/* Colours */
		...
		Joe_v1_2_Assets::js_inline('
			...
			if(typeof L !== "object" || L.version.indexOf("1") !== 1) {			
				...
				jQuery("head")
					.append(
						jQuery("<link />").attr({
							"href" : "' . Joe_v1_2_Helper::plugin_url('assets/css/leaflet.min.css') . '",
							"rel" : "stylesheet",
					...
					.append(
						jQuery("<script />").attr({
							"id" : "inmap_leaflet_js",					
							"src" : "' . Joe_v1_2_Helper::plugin_url('assets/js/leaflet.min.js') . '",
		...
		Joe_v1_2_Assets::js_enqueue([
			...
			'url' => Joe_v1_2_Helper::plugin_url('assets/js/shortcode.min.js'),
  			'deps' => [ 'jquery' ],
			'data' => [
				'basemap_url' => Joe_v1_2_Config::get_setting('appearance', 'map', 'basemap_url'),
				'basemap_attribution' => Joe_v1_2_Config::get_setting('appearance', 'map', 'basemap_attribution')
			]
		]);		
	}
	...
	public function handle_shortcode($shortcode_data, $content = null) {
		...
		$out = "\n" . '<!-- START ' . Joe_v1_2_Config::get_name() . ' Shortcode -->' . "\n";
		...
		), $shortcode_data, Joe_v1_2_Config::get_item('plugin_shortcode'));
		...
		if($shortcode_data['mapshare_identifier']) {					
			...
			//Error?
			if($error = Joe_v1_2_Log::in_error()) {
				Joe_v1_2_Log::render_item($error, 'console');			
			//Proceed
			} else {
				//Create *unqiue* Hash used to target Div
				$hash = Joe_v1_2_Helper::make_hash(
					array_merge(
						$Inreach_Mapshare->get_parameters(),
						//Salty count
						[
							'count' => Joe_v1_2_Log::get_data('shortcode_count')						
						]
					)
				);
				$map_div_id = 'inmap-' . $hash;
				Joe_v1_2_Log::add(__('Rendering Map', Joe_v1_2_Config::get_item('plugin_text_domain')) . ' (in Div #' . $map_div_id . ')', 'info', 'map_hash');				
				
				$geojson = $Inreach_Mapshare->get_geojson();

				if(is_string($geojson) && ! empty($geojson)) {		
					$point_count = $Inreach_Mapshare->get_point_count();
					$point_text = ($point_count == 1) ? __('Point', Joe_v1_2_Config::get_item('plugin_text_domain')) : __('Points', Joe_v1_2_Config::get_item('plugin_text_domain'));
							
					Joe_v1_2_Log::add(sprintf(__('Displaying %s MapShare', Joe_v1_2_Config::get_item('plugin_text_domain')), $point_count) . ' ' . $point_text, 'success', 'rendering_points');
					...
					if($route_url && $route_json = file_get_contents($route_url)) {
						...
						if(json_decode($route_json)) {
							Joe_v1_2_Log::add(__('Displaying route JSON.', Joe_v1_2_Config::get_item('plugin_text_domain')), 'success', 'route_valid');						
						} else {
							Joe_v1_2_Log::add(__('Invalid route JSON.', Joe_v1_2_Config::get_item('plugin_text_domain')), 'error', 'route_invalid');
						}						
					}
					
					//JS
					Joe_v1_2_Assets::js_onready('
						inmap_create_map(
							"' . $hash . '",
							' . $geojson . ',
							' . $route_json . '
					...
					$shortcode_count = (int)Joe_v1_2_Log::get_data('shortcode_count');
					$shortcode_count++;
					Joe_v1_2_Log::set_data('shortcode_count', $shortcode_count);				
				} else {
					Joe_v1_2_Log::add(__('GeoJSON contains no Points.', Joe_v1_2_Config::get_item('plugin_text_domain')), 'error', 'empty_geojson');				
				}
			}
		}	else {
			Joe_v1_2_Log::add(__('MapShare Identifier not provided.', Joe_v1_2_Config::get_item('plugin_text_domain')), 'error', 'missing_identifier');
		}

		$out .= '</div>';
		$out .= '<!-- END ' . Joe_v1_2_Config::get_name() . ' Shortcode -->' . "\n\n";
		
		//Log?
		
		//Display Full log to admin
		if(Joe_v1_2_Helper::do_debug() && current_user_can('administrator')) {
			Joe_v1_2_Log::render();
		//Error?
		} elseif($error = Joe_v1_2_Log::in_error()) {
			Joe_v1_2_Log::render_item($error);
		}
