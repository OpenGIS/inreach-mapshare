...
class Joe_v1_3_Helper {
	...
	static public function site_url($url_path = '') {
		return Joe_v1_3_Config::get_item('site_url') . $url_path;
	}
	...
	static public function plugin_url($file_path = '', $plugin_slug = '') {	
		if(! $plugin_slug) {
			$plugin_slug = Joe_v1_3_Config::get_item('plugin_slug');
		}
	...
	static public function plugin_file_path($file_path = '', $plugin_slug = '') {	
		if(! $file_path) {
			$file_path = Joe_v1_3_Config::get_item('plugin_slug') . '.php';
		}
	...
	static public function plugin_name($short = false) {
		if(! $short) {
			return Joe_v1_3_Config::get_item('plugin_name');
		} else {
			return Joe_v1_3_Config::get_item('plugin_name_short');
		}
	}

	static public function plugin_about() {
		$out = '	<div id="' . Joe_v1_3_Helper::css_prefix('about') . '">' . "\n";		

		$out .= '		<h1>' . Joe_v1_3_Config::get_item('plugin_name') . '</h1>' . "\n";

		$out .= Joe_v1_3_Config::get_item('plugin_about');

		$out .= '		<div class="' . Joe_v1_3_Helper::css_prefix('footer') . '">' . "\n";
		$out .= '			<div class="' . Joe_v1_3_Helper::css_prefix('joe') . '">' . "\n";
		$out .= '				<img alt="Joe\'s mug" src="https://www.morehawes.co.uk/assets/images/Joe1BW.jpg" />' . "\n";


		$out .= '				<p class="joe-lead">' . sprintf(__('Hi, I\'m <a href="%s">Joe</a>', Joe_v1_3_Config::get_item('plugin_text_domain')), 'https://www.morehawes.co.uk/') . '</p>' . "\n";	

		$out .= '				<p>' . __('This software is open source and maintained voluntarily, please help by contributing however you can.', Joe_v1_3_Config::get_item('plugin_text_domain')) . '</p>' . "\n";	
		$out .= '				<p>' . __('Each question, suggestion, bug report, review, translation and pull request is greatly appreciated.', Joe_v1_3_Config::get_item('plugin_text_domain')) . '</p>' . "\n";	

		$out .= '				<ul>' . "\n";		
	
		//WP.org Directory Link
		if(Joe_v1_3_Config::get_item('directory_url') && $directory_url = parse_url(Joe_v1_3_Config::get_item('directory_url'))) {
			if(isset($directory_url['host']) && ! empty($directory_url['host'])) {
				$out .= '					<li><a href="' . Joe_v1_3_Config::get_item('directory_url') . '">' . $directory_url['host'] . '</a></li>' . "\n";						
			}
		}

		//GitHub Repo Link
		if($github_url = parse_url(Joe_v1_3_Config::get_item('github_url'))) {
			if(isset($github_url['host']) && ! empty($github_url['host'])) {
				$out .= '					<li><a href="' . Joe_v1_3_Config::get_item('github_url') . '">' . $github_url['host'] . '</a></li>' . "\n";						
			}
		}

		$out .= '				</ul>' . "\n";		

		$out .= '				<small>v' . Joe_v1_3_Config::get_item('plugin_version') . '</small>' . "\n";
		$out .= '			</div>' . "\n";
		$out .= '		</div>' . "\n";
		$out .= '	</div>' . "\n";		
	...
	static public function do_debug() {
		return 
			...
			||
			(Joe_v1_3_Config::get_setting('joe', 'debug', 'enabled')	 == 1)
		;
	}
	...
	public static function convert_values_to_single_value($array_in) {
		...
		foreach($array_in as $key => $value) {
			...
			//Multiple values
				...
				$array_out[$key] = implode(Joe_v1_3_Config::get_item('multi_value_seperator'), $value);
			}
		}	
	...
	public static function convert_single_value_to_array($value_in) {
		...
		if(is_array($value_in)) {
			...
			foreach($value_in as $key => $value) {
				$multi = explode(Joe_v1_3_Config::get_item('multi_value_seperator'), $value);			
		...
		//String
			...
			return explode(Joe_v1_3_Config::get_item('multi_value_seperator'), $value_in);			
		}
	}		
	...
	public static function css_prefix($text = '')	{
		return Joe_v1_3_Config::get_item('css_prefix') . $text;
	}
	...
	static public function repeatable_setting_option_array($tab, $section, $key) {
		$options_array = array();
		$values = Joe_v1_3_Config::get_item($tab, $section, true);
		...
		foreach($values as $s) {
			...
			if(array_key_exists($key, $s)) {
				//Add as option
				$options_array[Joe_v1_3_Helper::make_key($s[$key])] = $s[$key];
			}		
		}
	...
	public static function assoc_array_table($assoc_array) {
		...
		$table = '<table class="' . Joe_v1_3_Helper::css_prefix('assoc_array') . '">';
					
		foreach($assoc_array as $key => $value) {
			$table .= '<tr class="' . Joe_v1_3_Helper::css_prefix('assoc_array-' . Joe_v1_3_Helper::make_key($key)) . '">';
			$table .= '<th>' . $key . '</th>';
			$table .= '<td>' . $value . '</td>';
			$table .= '</tr>';
		}
	...
	static function time_ago($time = '0', $comparison = false) {
		$periods_singular = [
			__('Second', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Minute', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Hour', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Day', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Week', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Month', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Year', Joe_v1_3_Config::get_item('plugin_text_domain'))
		];
		
		$periods_plural = [
			__('Seconds', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Minutes', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Hours', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Days', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Weeks', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Months', Joe_v1_3_Config::get_item('plugin_text_domain')),
			__('Years', Joe_v1_3_Config::get_item('plugin_text_domain'))
		];
		...
		$tense = __('ago', Joe_v1_3_Config::get_item('plugin_text_domain'));
