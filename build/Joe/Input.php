...
class Joe_v1_0_Input {
	...
	static public function create_field($field, $set_value = null, $show_label = true) {
		...
		if($field['type'] == 'boolean') {
			...
			$field['class'] .= Joe_v1_0_Helper::css_prefix() . 'short-input';
					
			if(! array_key_exists('options', $field) || ! is_array($field['options'])) {
				$field['options'] = array(
					'1' => esc_attr__('Yes', Joe_v1_0_Config::get_item('plugin_text_domain')),
					'0' => esc_attr__('No', Joe_v1_0_Config::get_item('plugin_text_domain'))
				);
			}
		}		
		...
		//Container
		$out .= '<div class="' . Joe_v1_0_Helper::css_prefix() . 'control-group ' . Joe_v1_0_Helper::css_prefix() . 'control-type-' . $field['type'] . $add_class . '">' . "\n";
	
		//Label
		if($show_label && isset($field['title'])) {
			$out .= '	<label class="' . Joe_v1_0_Helper::css_prefix() . 'control-label" for="' . $field['name'] . '">' . $field['title'] .  '</label>' . "\n";		
		}
		$out .= '	<div class="' . Joe_v1_0_Helper::css_prefix() . 'controls">' . "\n";				
		...
		if(isset($field['required']) && $field['required']) {	
			...
			if(in_array($field['type'], [ 'text' ]) && ! $is_colour) {
				...
				$field['tip'] .= __('Leave blank to restore the default.', Joe_v1_0_Config::get_item('plugin_text_domain'));
			}
		}
		...
		if(array_key_exists('tip', $field)) {
			...
			if(array_key_exists('tip_link', $field)) {
				$out .= ' ' . esc_attr__('Click here for more details.', Joe_v1_0_Config::get_item('plugin_text_domain')) . '" href="' . $field['tip_link'] . '" target="_blank" class="' . Joe_v1_0_Helper::css_prefix() . 'tooltip ' . Joe_v1_0_Helper::css_prefix() . 'link"';					
			} else {
				$out .= '" href="#" onclick="return false;" class="' . Joe_v1_0_Helper::css_prefix() . 'tooltip"';
			}
			$out .= '>?</a>';
		}
	...
	static public function create_input($field, $set_value = null) {
		...
		if(array_key_exists('default', $field)) {
			if(is_array($field['default'])) {
				$field['default'] = implode(Joe_v1_0_Config::get_item('multi_value_seperator'), static::process_output($field, $field['default']));				
			} else {
		...
		if($set_value !== null) {
			if(is_array($set_value)) {
				$set_value = implode(Joe_v1_0_Config::get_item('multi_value_seperator'), static::process_output($field, $set_value));				
			} else {
		...
		switch($field['type']) {
			...
			case 'select' :
				...
				$out .= '		<select data-multi-value="' . $set_value . '" class="' . Joe_v1_0_Helper::css_prefix() . 'input ' . Joe_v1_0_Helper::css_prefix() . 'input-' . $field['id'] . '" name="' . $field['name'] . '" data-id="' . $field['id'] . '">' . "\n";
			...
			case 'select_multi' :
				...
				} elseif(is_string($set_value) && strpos($set_value, Joe_v1_0_Config::get_item('multi_value_seperator'))) {
					$set_value = explode(Joe_v1_0_Config::get_item('multi_value_seperator'), $field['default']);
				}
				
				$out .= '		<select multiple="multiple" class="' . Joe_v1_0_Helper::css_prefix() . 'input ' . Joe_v1_0_Helper::css_prefix() . 'input-' . $field['id'] . '" name="' . $field['name'] . '[]" data-id="' . $field['id'] . '">' . "\n";
				...
				break;					
			case 'textarea' :
				$out .= '		<textarea class="' . Joe_v1_0_Helper::css_prefix() . 'input ' . Joe_v1_0_Helper::css_prefix() . 'input-' . $field['id'] . '" name="' . $field['name'] . '" data-id="' . $field['id'] . '">';
				//Do we have a value for this post?
			...
			case 'submit' :
				...
				$out .= '		<input type="submit" name="' . $field['name'] . '" value="' . $value . '" data-id="' . $field['id'] . '" class="' . Joe_v1_0_Helper::css_prefix() . 'input ' . Joe_v1_0_Helper::css_prefix() . 'input-' . $field['id'] . ' button-secondary" />' . "\n";
				
				break;				
			case 'file' :
				$out .= '		<input class="' . Joe_v1_0_Helper::css_prefix() . 'input ' . Joe_v1_0_Helper::css_prefix() . 'input-' . $field['id'] . '" type="file" name="' . $field['name'] . '" data-id="' . $field['id'] . '" />' . "\n";
			...
			default :
				...
				//Class
				$class = Joe_v1_0_Helper::css_prefix('input') 
					. ' ' . Joe_v1_0_Helper::css_prefix('input-' . $field['id'])
				;
...
// 		$out .= '<div class="' . Joe_v1_0_Helper::css_prefix() . 'repeatable-container" data-count="' . $count . '">' . "\n";
// 		
// 		$out .= $repeatable_parameter_groups;
// 
// 		//Template
// 		$out .= static::create_parameter_groups($fields, $groups, [], $name . '[__count__][%s]', '', Joe_v1_0_Helper::css_prefix() . 'repeatable-template');			
// 
// 		$out .= '<button class="button ' . Joe_v1_0_Helper::css_prefix() . 'repeatable-add" title="' . __('Add Query', Joe_v1_0_Config::get_item('plugin_text_domain')) . '"><i class="ion ion-plus"></i></button>';
// 
// 		$out .= '</div>' . "\n";
...
// 	}
	...
	static function create_parameter_groups($fields, $groups = array(), $data = array(), $input_name_format = null, $id = '', $class_append = '') {				
		...
		$out .= '<div' . $id . ' class="' . Joe_v1_0_Helper::css_prefix() . 'parameters-container ' . Joe_v1_0_Helper::css_prefix() . 'accordion-container' . $class_append . '">' . "\n";
		...
		foreach($fields as $group_id => $fields) {
			...
			if($by_group && ($current_group != $group_id)) {
				...
				$out .= '	<div class="' . Joe_v1_0_Helper::css_prefix() . 'parameter-group ' . Joe_v1_0_Helper::css_prefix() . 'accordion-group ' . Joe_v1_0_Helper::css_prefix() . 'parameter-group-' . $group_id . '" id="' . Joe_v1_0_Helper::css_prefix() . 'parameter-group-' . $group_id . '">' . "\n";					
				$out .= '		<legend title="Click to expand">' . $group['group_title'] . '</legend>' . "\n";
				$out .= '		<div class="' . Joe_v1_0_Helper::css_prefix() . 'accordion-group-content">' . "\n";
				if(array_key_exists('group_description', $group)) {			
					$out .= '			<p class="' . Joe_v1_0_Helper::css_prefix() . 'parameter-group-description">' . $group['group_description'] . '</p>' . "\n";
				}
	...
	static function get_file_contents($file) {
		...
		if(is_uploaded_file($file['tmp_name'])) {	
			...
			//Not allowable file
				...
				$response['error'] = esc_html__('The file extension uploaded is not supported.', Joe_v1_0_Config::get_item('plugin_text_domain'));		
	...
	public static function allowable_file($ext = '', $mime = '', $file_image = 'file') {
		$allowable_mimes = Joe_v1_0_Config::get_item('mimes', $file_image);
