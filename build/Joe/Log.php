...
class Joe_v1_3_Log {
	...
	public static function out($content = '') {
		switch(static::$output_type) {
			case 'notice' :
				$latest = Joe_v1_3_Log::latest();
				$type = isset($latest['type']) ? $latest['type'] : '';
				if(static::$in_success && static::latest_warning()) {
					$type = 'warning';
				}
				
				Joe_v1_3_Assets::js_onready('joe_admin_message("' . $content . '", "' . $type . '")');
		
				break;

			default :
			case 'console' :
				Joe_v1_3_Assets::js_inline('console.log("' . strip_tags($content) . '");');
	...
	public static function render() {
		...
		//Not debugging
		if(! Joe_v1_3_Helper::do_debug() && $latest = Joe_v1_3_Log::latest()) {
	...
	public static function draw_item(array $item) {
		...
		switch(static::$output_type) {
			...
			case 'console' :
				...
				return '[' . Joe_v1_3_Config::get_name() . ' ' . ucwords($item['type']) . $code . '] ' . $item['message'] . '\n';
		}
