...
class Joe_v1_3_Cache {
	
	static function set_item($cache_id, string $cache_content, int $cache_minutes = 0) {
		if(! $cache_minutes) {
			$cache_minutes = (int) Joe_v1_3_Config::get_setting('joe', 'cache', 'minutes');
		}
		$cache_seconds = $cache_minutes * 60;

		set_transient(Joe_v1_3_Helper::slug_prefix($cache_id, '_', false), $cache_content, $cache_seconds);						
	}
	
	static function get_item($cache_id, $check_stale = false) {
		//Straight WP caching baby...
		if(! $check_stale) {
			return get_transient(Joe_v1_3_Helper::slug_prefix($cache_id, '_', false));
		//Joe implementation
		} else {	
			global $wpdb;
			...
			$results = $wpdb->get_results(
				...
				", '_transient_%' . Joe_v1_3_Helper::slug_prefix($cache_id, '_', false)
				)
			, ARRAY_A);
			...
			if(sizeof($results) == 2) {
				...
				foreach($results as $result) {
					if($result['option_name'] == '_transient_timeout_' . Joe_v1_3_Helper::slug_prefix($cache_id, '_', false)) {
						$timeout = $result['option_value']; 			
					} elseif($result['option_name'] == '_transient_' . Joe_v1_3_Helper::slug_prefix($cache_id, '_', false)) {
