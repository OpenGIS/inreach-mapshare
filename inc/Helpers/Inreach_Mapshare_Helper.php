...
class Inreach_Mapshare_Helper {
	...
	static public function plugin_about() {
		...
		$out .= '	<div id="' . Inreach_Mapshare_Config::get_item('plugin_slug') . '-about">' . "\n";	
		$out .= '		<img alt="Joe\'s mug" src="https://www.morehawes.co.uk/assets/images/Joe1BW.jpg" />' . "\n";		
		$out .= '		<p class="lead"><b>' . sprintf(esc_html__("Hi, I'm %s.", Inreach_Mapshare_Config::get_item('plugin_slug')), "Joe") . '</b></p>' . "\n";		
		$out .= '	</div>' . "\n";		
	...
	static public function logo($colour = 'dark', $width = '20', $height = '20', $title = false) {
		if(! $title) {
			$title = Inreach_Mapshare_Config::get_name();
		}
		return '<img class="' . Inreach_Mapshare_Config::get_item('plugin_slug') . '-logo" alt="' . Inreach_Mapshare_Config::get_name() . '" src="' . self::asset_url('img/' . Inreach_Mapshare_Config::get_item('plugin_slug') . '-icon-' . $colour . '.png') . '" width="' . $width . '" height="' . $height . '" />';
	}
	
	static public function site_url($url_path = '') {
		return Inreach_Mapshare_Config::get_item('site_url') . $url_path;
	}

	static public function asset_url($file_path = '') {	
		return plugin_dir_url('') . Inreach_Mapshare_Config::get_item('plugin_slug') . '/assets/' . $file_path;
	}
	...
	static public function http_url($data = array()) {
		return trim(add_query_arg(array_merge(array(Inreach_Mapshare_Config::get_item('plugin_slug') . '_http' => '1'), $data), home_url('/')), '/');
	}
	...
	static public function is_debug() {
		return (true == Inreach_Mapshare_Config::get_setting('misc', 'advanced', 'debug_mode'));
	}
...
// 		}
	...
	public static function convert_single_value_to_array($value_in) {
		...
		if(is_array($value_in)) {
			...
			foreach($value_in as $key => $value) {
				$multi = explode(Inreach_Mapshare_Config::get_item('multi_value_seperator'), $value);			
		...
		//String
			...
			return explode(Inreach_Mapshare_Config::get_item('multi_value_seperator'), $value_in);			
		}
	}	
	...
	public static function convert_values_to_single_value($array_in) {
		...
		foreach($array_in as $key => $value) {
			...
			//Multiple values
				...
				$array_out[$key] = implode(Inreach_Mapshare_Config::get_item('multi_value_seperator'), $value);
			}
		}	
	...
	public static function assoc_array_table($assoc_array) {
		...
		$table = '<table class="' . Inreach_Mapshare_Config::get_item('plugin_slug') . '-assoc_array">';
					
		foreach($assoc_array as $key => $value) {
			$table .= '<tr class="' . Inreach_Mapshare_Config::get_item('plugin_slug') . '-assoc_array-' . $key . '">';
			$table .= '<th>' . $key . '</th>';
			$table .= '<td>' . $value . '</td>';
			$table .= '</tr>';
		}
