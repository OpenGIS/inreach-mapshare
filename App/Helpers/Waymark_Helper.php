...
class Waymark_Helper {
	...
	static public function logo($colour = 'dark', $width = '20', $height = '20', $title = false) {
		if(! $title) {
			$title = Joe_Config::get_name();
		}
		return '<img class="waymark-logo" alt="' . Joe_Config::get_name() . '" src="' . self::asset_url('img/waymark-icon-' . $colour . '.png') . '" width="' . $width . '" height="' . $height . '" />';
	}
	...
	static public function get_map_meta($Map, $context = 'map_single') {
		...
		$settings_meta = Joe_Config::get_item('meta', 'inputs', true);		
		...
		foreach($settings_meta as $setting_meta) {
			...
			$meta_key = Joe_Helper::make_key($setting_meta['meta_title'], 'map');
			...
			if(array_key_exists($meta_key, $Map->data) && ! empty($Map->data[$meta_key])) {
				...
				if(in_array($setting_meta['meta_type'], array('select')) && array_key_exists('meta_options', $setting_meta)) {
					$options_array = Joe_Helper::comma_string_to_array($setting_meta['meta_options']);
				...
				//Multi-Select				
					...
					$options_array = Joe_Helper::comma_string_to_array($setting_meta['meta_options']);					
					$values_array = Joe_Helper::array_string_to_array($Map->data[$meta_key]);
		...
		//Display Collections?
		if(Joe_Config::get_setting('misc', 'collection_options', 'link_from_maps')) {
...
// 					}				
		...
		if($has_features && Joe_Config::get_setting('misc', 'map_options', 'allow_export') == true) {			
...
// 		Joe_Helper::debug($map_meta);
	...
	static public function get_meta_groups() {
		//Get Groups
		$meta_groups = Joe_Config::get_item('meta', 'groups', true);
		$meta_groups = Joe_Helper::multi_use_as_key($meta_groups, 'group_title');	
	...
	static public function is_debug() {
		return (true == Joe_Config::get_setting('misc', 'advanced', 'debug_mode'));
	}
...
// 		}
	...
	static public function add_map_link_to_description($map_id, $map_title = false, $map_data) {
		...
		return Waymark_GeoJSON::set_features_property($map_data, 'description', $desc_append, true);												
	}
	...
	public static function get_object_types($type = 'marker', $use_key = false, $as_options = false) {
		...
		$object_types = Joe_Config::get_item($type . 's', $type . '_types', true);

		//Use keys
		if(is_string($use_key)) {
			$object_types = Joe_Helper::multi_use_as_key($object_types, $use_key);			

			//Convert to dropdown
			if($as_options) {
				foreach($object_types as $key => $data) {

					$types_out[Joe_Helper::make_key($key, '', false)] = $data[$use_key];					
				}
			}
		//Don't modify
		}	else {
	...
	static public function map_export_html($Map) {
		...
		$out  = '<' . $element . ' action="' . admin_url('admin-ajax.php') . '" method="post" id="waymark-map-export-' . $Map->post_id . '" class="waymark-map-export" data-map_id="' . $Map->post_id . '" data-map_slug="' . sanitize_title($Map->post_title) . '">' . "\n";
		...
		$out .= '	</select>' . "\n";
		$out .= '	<input type="hidden" name="action" value="waymark_download_map_data" />' . "\n";
		$out .= '	<input type="hidden" name="waymark_security" value="' . wp_create_nonce(Joe_Config::get_item('nonce_string')) . '" />' . "\n";
	...
	public static function get_settings_js() {
		$settings = get_option(Joe_Config::get_item('settings_id'));
	...
	public static function get_map_config() {
		...
		//Basemaps
		$tile_layers = Joe_Config::get_item('tiles', 'layers');
		$tile_layers = Joe_Helper::convert_values_to_single_value($tile_layers);	
		//Process for output
		foreach($tile_layers as &$tl) {
			$tl = htmlspecialchars_decode($tl);		
		}
		$tile_layers = Joe_Helper::convert_single_value_to_array($tile_layers);
		$map_config['tile_layers'] = $tile_layers;
		
		//Object Types	
		$map_config['marker_types'] = self::get_object_types('marker');
		$map_config['line_types'] =  self::get_object_types('line');
		$map_config['shape_types'] = self::get_object_types('shape');

		//Editor Options
		$map_config['map_options'] = array(
			'show_type_labels' => Joe_Config::get_setting('misc', 'map_options', 'show_type_labels'),
			'button_position' => 'topleft'			
		);

		//Editor Options
		$map_config['editor_options'] = array(
			'confirm_delete' => Joe_Config::get_setting('misc', 'editor_options', 'confirm_delete')
		);
